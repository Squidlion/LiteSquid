...
#if defined(NDEBUG)
# error "Litesquid cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4c3deeb3f9c462b5215c63f8ff06729c33bb226553ee59c635dc77494c221566");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Litesquid: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Litesquid Signed Message:\n";
...
{
    // Litesquid: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 2073600 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 2073600); // Litesquid: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Litesquid: 3 days
static const int64 nTargetSpacing = 1 * 60; // Litesquid: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Litesquid: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Litesquid: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x99498633ecaf90749db89fbef90423eaf514ce307e01b2bbb522529c0986f382");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Biden admin will provide Ukraine with longer-range rocket systems 5/31/2022";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("046386c378e2f373a54990d29ceb695aa24c92a100c0888f1641d719d126dd6ca0c439ed690fe97a7c0a16ac918b7b7888e12241a735aa8fabc361dc634a08d034") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1654055329;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084725099;

        if (fTestNet)
        {
            block.nTime    = 1654055235;
            block.nNonce   = 387620246;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x0d5e85dd9ecee5be8f61e08a8efa1be3e4bf654836021be4b1f0fa531aee6c2d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc1, 0xb9, 0xdc }; // Litesquid: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LitesquidMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LitesquidMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LitesquidMiner(CWallet *pwallet)
{
    printf("LitesquidMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("litesquid-miner");
    ...
    try { loop {
        ...
        printf("Running LitesquidMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LitesquidMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LitesquidMiner, pwallet));
}
